...
public class CNPJService {
	...
	public EnterpriseResponse findCNPJ(String CNPJ) {
		...
		Mono<EnterpriseResponse> mono = this.webClient.method(HttpMethod.GET).uri("/{cnpj}", CNPJ).retrieve()// retrieve
																												// dispatches
																												// the
																												// request
				.onStatus(HttpStatus::is4xxClientError, response -> {
					...
					return Mono.error(new ApiException(
							"A API da Receita impediu essa requisição" + "\n Motivo: Muitas requisições"));
